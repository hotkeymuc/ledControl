...
VERSION = 'ledControl'
screenSize = (1024, 600)
FPS = 10	# 10 fps is max for RAW-data timeout (if not enough bytes have been sent)
...
if __name__ == '__main__':
	...
	LEDS = 300
	lc = LEDControl_serial(num_leds=LEDS, order=ORDER_GRB)
	#lc = LEDControl_udp()
	#lc = LEDControl_http()
	...
	for i in range(LEDS):
		lc.leds[i].pos = [(p[j]) for j in range(3)]
		#p[0] += 10
		p[0] += sx
		if (p[0] > screenSize[0]):
			p[0] = 0
	...
	img = pygame.image.load('img/wallpaper-2012.jpg')
	#img = pygame.image.load('img/wallpaper-57118.jpg')
		...
		for event in pygame.event.get():
			...
			if event.type == MOUSEMOTION:
				...
				for led in lc.leds:
					...
					#led.color = [min(1.0, max(0.0, 1.0-(d/200))) for i in range(3)]
					
					#w = wheel(min(1.0, max(0.0, d/200)))
					#led.color = [min(1.0, max(0.0, w[i])) for i in range(3)]
			...
			if event.type == KEYDOWN:
				if (event.key in [282, 1073741882]):	#F1	#ord('0')):
					p = FxPreset('Black Out')
					p.addLayer(FxLayer(FxType.FX_BLACK,	1,0,	-1,-1,	0,0,0,0))
					lc.send(p.get_commands())
					
				elif (event.key in [283, 1073741883]):	#F2	# == ord('1')):
				...
				elif (event.key in [284, 1073741884]):	#F3	# == ord('2')):
				...
				elif (event.key in [285, 1073741885]):	#F4	# == ord('3')):
				...
				elif (event.key in [286, 1073741886]):	#F5	# == ord('4')):
				...
				elif (event.key in [287, 1073741887]):	#F6	# == ord('5')):
				...
				elif (event.key in [290, 1073741890]):	#F9	# == ord('9')):
				...
				elif (event.key in [293, 1073741893]):	#F12	# == ord('b')):
					p = FxPreset('Black Out')
					w = 268
					w2 = int(w/2)
